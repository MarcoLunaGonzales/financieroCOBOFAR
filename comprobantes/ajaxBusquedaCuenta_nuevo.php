	...
	<div class="table-responsive">
		...
		while ($row = $stmt->fetch(PDO::FETCH_BOUND)) {
			...
			while ($rowAux = $stmtAux->fetch(PDO::FETCH_BOUND)) {
				...
				<td class='text-left small'><a href='javascript:setBusquedaCuenta_nuevo(\"$codigoCuenta\",\"$numeroCuenta\",\"$nombreCuenta\",\"$codigoCuentaAux\",\"$nombreCuentaAux\");'>".strtoupper($nombreCuentaAux)."</a></td>
				</tr>";
				$contAux++;
			}  	
		...
		<tr>
			<td class="text-left"><?=$numeroCuenta;?></td>
	      	<td class="text-left"><a href="javascript:setBusquedaCuenta_nuevo('<?=$codigoCuenta;?>','<?=$numeroCuenta;?>','<?=$nombreCuenta;?>','0','');"><?=$nombreCuenta;?></a></td>
