...
$stmtX->execute();
// Preparamos
$stmt = $dbh->prepare("SELECT (select u.nombre from unidades_organizacionales u where u.codigo=c.cod_unidadorganizacional)unidad,c.cod_unidadorganizacional as cod_unidad,c.cod_gestion, 
...
$stmt = $dbh->prepare("SELECT (select e.nombre from entidades e where e.codigo=cod_entidad)as cod_entidad from entidades_uo where cod_uo=$codUC;");
// Ejecutamos
$stmt->execute();
$stmt->bindColumn('cod_entidad', $nombre_entidad);
while ($rowDetalle = $stmt->fetch(PDO::FETCH_BOUND)) {
    $nombre_entidadC=$nombre_entidad;  
}
           ...
           '<tbody>';
            ...
            while ($row = $data->fetch(PDO::FETCH_ASSOC)) {
             ...
             $html.='<tr>'.
                      ...
                      '<td>'.$row['nombre'].'-'.$row['nombrecuentaauxiliar'].'('.$row['cod_det'].')<br>'.$row['glosa'].'</td>';
