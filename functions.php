<?php
  ...
  function obtieneNuevaCuenta($codigo){//ESTA FUNCION TRABAJA CON UNA CUENTA FORMATEADA CON PUNTOS
     ...
     $cod_padre=obtieneCuentaPorNumero($cuentaSinFormato);
     list($nivel1, $nivel2, $nivel3, $nivel4, $nivel5) = explode('.', $codigo);
     $sql="SELECT (max(numero))numero FROM plan_cuentas where cod_padre=$cod_padre";
     ...
     //echo "<br><br><br><br>max:".$cuentaHijoMaxima;
     //ACA SACAMOS EL NUMERO DEL NIVEL MAXIMO
     $numeroIncrementar=0;
     if($nivelCuentaBuscado==2){
        $numeroIncrementar=substr($cuentaHijoMaxima, 1,1);
     }
     if($nivelCuentaBuscado==3){
        $numeroIncrementar=substr($cuentaHijoMaxima, 2,2);
     }
     if($nivelCuentaBuscado==4){
        $numeroIncrementar=substr($cuentaHijoMaxima, 4,2);
     }
     if($nivelCuentaBuscado==5){
        $numeroIncrementar=substr($cuentaHijoMaxima, 6,3);
     }
  ...
  function formateaPuntosPlanCuenta($cuenta){
    ...
    $nivel2=substr($cuenta, 1, 1);
    $nivel3=substr($cuenta, 2, 2);
    $nivel4=substr($cuenta, 4, 2);
    $nivel5=substr($cuenta, 6, 3);
  ...
  function nameProveedor($codigo){
   ...
   function nameProveedor_antsist($codigo){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT nombre FROM af_proveedores where idproveedor_almacen=:codigo");
     $stmt->bindParam(':codigo',$codigo);
     $stmt->execute();
     $nombreX="";
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  function codigoProveedorNuevo($codigo){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT codigo FROM af_proveedores where idproveedor_almacen=:codigo");
     $stmt->bindParam(':codigo',$codigo);
     $stmt->execute();
     $codigoX="";
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  function abrevTipoBienes($codigo){
   $dbh = new Conexion();
   $stmt = $dbh->prepare("SELECT abreviatura FROM tiposbienes where codigo in ($codigo)");
   $stmt->execute();
   $nombreX="";
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
     $nombreX.=$row['abreviatura'];
...
function obtenerTotalesPlantillaServicio($codigo,$tipo,$mes){
  ...
  where pc.codigo=$codigo and pgc.cod_tiposervicio=1 GROUP BY pgd.cod_plantillagruposervicio order by pgd.cod_plantillagruposervicio";
   ...
   function obtenerValorConfiguracion_array($String_id){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT valor_configuracion from configuraciones c where id_configuracion in ($String_id)");
     $stmt->execute();
     $codigoComprobante="";
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $codigoComprobante.=$row['valor_configuracion'].",";
     }
     $codigoComprobante=trim($codigoComprobante,",");
     return($codigoComprobante);
   }
...
function obtenerCorrelativoComprobante2($cod_tipocomprobante){  
  ...
  function descargarPDFHorizontal_boletas($nom,$html){
    ...
    require_once 'assets/libraries/dompdf/dompdf_config.inc.php';
    $mydompdf = new DOMPDF();
    $mydompdf->set_paper('A4', 'landscape');
    ob_clean();
    $mydompdf->load_html($html);
    $mydompdf->render();
    $canvas = $mydompdf->get_canvas();
  ...
  function nameTipoCuentaAuxiliar($codigo){
    $nombreTipoAuxiliar="";
    $dbh = new Conexion();
    $stmt = $dbh->prepare("SELECT nombre from tipos_estado_cuenta where codigo=$codigo order by nombre");
   $stmt->execute();
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
      $nombreTipoAuxiliar=$row['nombre'];
   }
 ...
 function obtenerCodigoCuentaAuxiliarProveedorClienteCuenta($tipo,$codigo,$cuenta){
  ...
  function formatoNumeroCuenta($numero){ 
   return $numero[0].".".$numero[1].".".$numero[2].$numero[3].".".$numero[4].$numero[5].".".$numero[6].$numero[7].$numero[8]; 
  }
  ...
  function obtenerCodigoEstadoCuentaSolicitudRecursosDetalle($codigo){
   ...
   function obtenerCodigoEstadoCuentaRelacionado($codigo){
    $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT p.cod_estadocuenta from solicitud_recursosdetalle p where p.codigo=$codigo");
     $stmt->execute();
     $valor="";
  ...
  function obtenerDatosProveedoresPagoDetalle($codigo){
    // include 'solicitudes/configModule.php';
     $dbh = new Conexion();
     $sql="SELECT pd.*,e.glosa_auxiliar,e.fecha as fecha_ec,c.numero,c.cod_unidadorganizacional,c.codigo as cod_comprobante
      from pagos_proveedoresdetalle pd join estados_cuenta e on pd.cod_solicitudrecursos=e.codigo join comprobantes_detalle cd on e.cod_comprobantedetalle=cd.codigo join comprobantes c on c.codigo=cd.cod_comprobante
     ...
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $detalles[$index]=$row['glosa_auxiliar'];
        if(($index % 2) == 0){
          $estiloBoton="btn-warning";
        }else{
          $estiloBoton="btn-danger";
        }
        $fechaSol[$index]=strftime('%d/%m/%Y',strtotime($row['fecha_ec']));
        $numero[$index]="<a title='ver Comprobante' href='#' target='_blank' class=''>".$row['numero']."</a>";
...
function anular_pago_curso($ci_estudiante,$IdCurso,$Idmodulo,$monto,$cod_solfac){
  ...
  function obtenerPagoProveedorDetalle($codigo){
     $dbh = new Conexion();
     $sql="";
     $sql=" SELECT pp.codigo,ppd.cod_proveedor,ppd.cod_tipopagoproveedor,ppd.monto,ppd.observaciones,DATE_FORMAT(ppd.fecha,'%d/%m/%Y') as fecha,ppd.pronto_pago from pagos_proveedores pp join pagos_proveedoresdetalle ppd on ppd.cod_pagoproveedor=pp.codigo
   where pp.cod_pagolote=$codigo";
     $stmt = $dbh->prepare($sql);
     $stmt->execute();
     return $stmt;
...
function obtenerObtenerLibretaBancariaIndividualAnio($codigo,$anio,$fecha,$monto,$nombre){
  ...
  function obtenerCodigoPagoLote(){
    ...
    function obtenerCorrelativoPagoLote(){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT IFNULL(max(c.nro_correlativo)+1,1)as codigo from pagos_lotes c");
     $stmt->execute();
     $codigo=1;
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $codigo=$row['codigo'];
   ...
   function obtenerdatosCheque($codigo){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT codigo,numero from cheques_emitidos where cod_pagodetalle=$codigo and cod_estadoreferencial=1");
     $stmt->execute();
     $valor="";
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['codigo'].",".$row['numero'];
     }
     return($valor);
  }
...
where e.cod_comprobantedetalleorigen=0 and e.cod_comprobantedetalle in (SELECT cd.codigo from comprobantes_detalle cd join comprobantes c on c.codigo=cd.cod_comprobante where c.codigo=$codigo and c.cod_estadocomprobante<>2))";
   ...
   if($valor==0){
      //$valor=existeComprobanteRelacionadoingresosSuc($codigo);
   }
     return($valor);
  }
...
function obtenerCodPersonal2_asignacion($codigo){
  $dbh = new Conexion();
  $stmt = $dbh->prepare("SELECT cod_personal2 from activofijos_asignaciones where codigo=$codigo");
  $stmt->execute();
  $valorX="";
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $valorX=$row['cod_personal2'];
  }
  return($valorX);
}
function obtenercodigoalterno_tipobienes($codigo){
  $dbh = new Conexion();
  $stmt = $dbh->prepare("SELECT codigo_alterno from tiposbienes where codigo=$codigo");
  $stmt->execute();
  $valorX=0;
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $valorX=$row['codigo_alterno'];
  }
  return($valorX);
}

function obtener_codcorrelativo_alterno_tipobien($codigo){
  $dbh = new Conexion();
  $sql="SELECT codigo_alterno from tiposbienes where  cod_depreciaciones=$codigo order by codigo_alterno desc limit 1";
  $stmt = $dbh->prepare($sql);
  $stmt->execute();
  $valorX=1;
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function obtenerdatos_af_asignacion($codigo){
  $dbh = new Conexion();
  $sql="SELECT fechaasignacion,estadobien_asig from activofijos_asignaciones where cod_activosfijos=$codigo and cod_estadoasignacionaf=2 order by codigo desc limit 1";
  $stmt = $dbh->prepare($sql);
  $stmt->execute();
  $valorX="";
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $valorX=$row['fechaasignacion']."###".$row['estadobien_asig'];
  }
  return($valorX);
}

function obtenerNombreTurno($codigo){
  $dbh = new Conexion();
  $sql="SELECT nombre from personal_turno where  codigo=$codigo";
  $stmt = $dbh->prepare($sql);
  $stmt->execute();
  $valorX="";
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function obtenerNombreTipoTrabajo($codigo){
  $dbh = new Conexion();
  $sql="SELECT nombre from personal_tipotrabajo where  codigo=$codigo";
  $stmt = $dbh->prepare($sql);
  $stmt->execute();
  $valorX="";
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function obtenerCodigoReferencialActivo($cod_tiposactivos,$cod_depreciaciones,$cod_tiposbienes){
   $dbh = new Conexion();
   $cod_tipo_fungible=obtenerValorConfiguracion(102);//variableConfiguracion
   if($cod_tiposactivos==$cod_tipo_fungible){//FUNGIBLE
      $sqlCodAF="SELECT codigoactivo From activosfijos where tipo_af='$cod_tiposactivos' order by codigo desc limit 1";
      $stmtCodAF = $dbh->prepare($sqlCodAF);
      //echo $sqlCodAF;
      $stmtCodAF->execute();
      ...
      $sqlCodAF="SELECT codigoactivo From activosfijos  where cod_depreciaciones=$cod_depreciaciones and cod_tiposbienes=$cod_tiposbienes and tipo_af=$cod_tiposactivos order by codigoactivo desc limit 1";
      $stmtCodAF = $dbh->prepare($sqlCodAF);
      $stmtCodAF->execute();
...
function obtenerComprobantePago($codigo){
   $dbh = new Conexion();
   $sql="SELECT c.codigo from comprobantes c where codigo in (select p.cod_comprobante from pagos_lotes p where p.codigo=$codigo) and c.cod_estadocomprobante=1";
   $stmt = $dbh->prepare($sql);
   $stmt->execute();
   $valorX=0;
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function obtenerMontoPagoLote($codigo_pago,$cod_estadocuenta){
   $dbh = new Conexion();
   $sql="SELECT pd.monto
      from pagos_proveedores p INNER JOIN pagos_proveedoresdetalle pd on pd.cod_pagoproveedor=p.codigo
      where p.cod_pagolote=$codigo_pago and pd.cod_solicitudrecursos=$cod_estadocuenta";
   $stmt = $dbh->prepare($sql);
   $stmt->execute();
   $valorX=0;
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
  ...
  function obtnerDescuentoProveedor($cod_proveedor){
    $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT  descuento from af_proveedores where codigo=$cod_proveedor");
     $stmt->execute();
     $valor=0;
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['descuento'];
...
function obtenerIngresoPendienteDatos($codIngreso){
  ...
  function actualizarIngresoComprobante($codIngreso,$codComprobante){
   ...
   //funciones ingresos sucursales

   function obtenerMonto_ventas_nuevosis($fecha_iniconsultahora,$rpt_territorio,$cod_personal){
   ...
   function obtenerMontoTarjeta_ventas_nuevosis($fecha_iniconsultahora,$rpt_territorio,$cod_personal){
      if($cod_personal==-1000){//para reporte general
       $sql="SELECT s.`monto_final` as monto from `salida_almacenes` s 
   ...
   function obtenerMontoAnuladas_ventas_nuevosis($fecha_iniconsultahora,$rpt_territorio,$cod_personal){
     if($cod_personal==-1000){//para reporte general
       ...
       and s.fecha_anulacion BETWEEN '$fecha_iniconsultahora 06:00:00' and '$fecha_iniconsultahora 23:59:59' and s.cod_tipopago=1 ";
     }else{
       $sql="SELECT  s.monto_final as monto
       from `salida_almacenes` s where s.`cod_tiposalida`=1001 and s.salida_anulada!=0 and
       s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a where a.`cod_ciudad`='$rpt_territorio' and cod_tipoalmacen=1)
       and s.fecha_anulacion BETWEEN '$fecha_iniconsultahora 06:00:00' and '$fecha_iniconsultahora 23:59:59' and s.cod_chofer_anulacion=$cod_personal and s.cod_tipopago=1 ";  
     }
   ...
   function obtenerMontodepositado_general_nuevosis2($fecha,$cod_ciudad){
     $sql="SELECT s.cod_chofer from `salida_almacenes` s 
     where s.`cod_tiposalida`= 1001 and s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a where a.`cod_ciudad`='$cod_ciudad' and cod_tipoalmacen=1) and CONCAT(s.fecha,' ',s.hora_salida) BETWEEN '$fecha 00:00:00' and '$fecha 23:59:59' and s.cod_tipopago=1 
     ...
     while($row=mysqli_fetch_array($resp)){  
       ...
       $monto=obtenerMontodepositado_nuevosis($fecha,$cod_personal);
       $valor+=$monto;
     } 
      mysqli_close($dbh);
     return $valor;
   }
   function obtenerMontodepositado_dolar_general_nuevosis2($fecha,$cod_ciudad){
     $sql="SELECT s.cod_chofer from `salida_almacenes` s 
     where s.`cod_tiposalida`= 1001 and s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a where a.`cod_ciudad`='$cod_ciudad' and cod_tipoalmacen=1) and CONCAT(s.fecha,' ',s.hora_salida) BETWEEN '$fecha 00:00:00' and '$fecha 23:59:59' and s.cod_tipopago=1 
     GROUP BY s.cod_chofer,s.cod_tipopago,s.fecha order by s.fecha,s.cod_chofer ";
     // echo $sql;  
     ...
     while($row=mysqli_fetch_array($resp)){  
       $cod_personal=$row['cod_chofer'];
       $monto=obtenerMontodepositado_dolar_nuevosis($fecha,$cod_personal);
       $valor+=$monto;
     } 
      mysqli_close($dbh);
     return $valor;
   ...
   function obtenerMontodepositado_dolar_nuevosis($fecha,$cod_personal){
     $sql="SELECT monto_registradousd as monto from registro_depositos where fecha='$fecha' and cod_funcionario=$cod_personal and cod_estadoreferencial=1";
     //echo $sql;  
     $valor=0;
     require("conexion_comercial.php");
     $resp=mysqli_query($dbh,$sql);
   ...
   function obtener_codciudad_almacen_nuevosis($codigo_almacen,$tipo){
      $sql="SELECT cod_ciudad FROM almacenes where cod_almacen='$codigo_almacen' and cod_tipoalmacen=$tipo";
      //echo $sql;  
      $value=0;
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
   ...
   function obtener_codalmacen_nuevosis($cod_ciudad,$tipo){
      $sql="SELECT cod_almacen from almacenes where cod_ciudad = $cod_ciudad and cod_tipoalmacen=$tipo";
      $value=0;
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
      while($row=mysqli_fetch_array($resp)){     
      $value=$row['cod_almacen']; 
      } 
      mysqli_close($dbh);
      return $value;
   }
   function nombrePersonal_nuevosis($codigo)
   { 
      $sql="select concat(paterno,' ',nombres) as nombre from funcionarios where codigo_funcionario='$codigo'";
      $value="";
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
      while($row=mysqli_fetch_array($resp)){     
   ...
   function obtener_cuenta_moneda_extranjera_dolar_nuevosis($fecha,$rpt_territorio,$cod_personal){
      $sql="SELECT (SELECT (select cb.cod_plancuenta from cuentas_bancarias cb where cb.codigo=cr.cod_cuenta and cb.estado=1) as cuenta  from cuentas_registrodeposito  cr where cr.cod_registrodeposito=rd.codigo and cr.cod_cuenta<>rd.cod_cuenta) as cuenta_extrajera
      FROM registro_depositos rd 
      WHERE rd.fecha='$fecha' and rd.cod_estadoreferencial=1 and rd.cod_funcionario=$cod_personal and rd.cod_ciudad=$rpt_territorio";
      //echo $sql;  
      $valor=0;
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
      while($row=mysqli_fetch_array($resp)){    
         $valor=$row['cuenta_extrajera'];
      } 
      mysqli_close($dbh);
      return $valor;
   }
   function obtenerCostoVenta_nuevosis($fecha_iniconsultahora,$rpt_territorio,$cod_personal){
      if($cod_personal==-1000){//para reporte general
       // $sql="SELECT s.`monto_final` as monto from `salida_almacenes` s 
       // where s.`cod_tiposalida`=1001 and s.salida_anulada=0 and s.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a
       // where a.`cod_ciudad`='$rpt_territorio' and cod_tipoalmacen=1) and s.fecha = '$fecha_iniconsultahora' and s.cod_tipopago=2";
      }else{
       $sql="SELECT sum(sad.costo_almacen) as costo_venta
         from salida_almacenes sa INNER JOIN salida_detalle_almacenes sad on sad.cod_salida_almacen=sa.cod_salida_almacenes
         where sa.fecha = '$fecha_iniconsultahora' and sa.cod_tiposalida=1001 and sa.salida_anulada=0 and sa.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a
       where a.`cod_ciudad`='$rpt_territorio' and a.cod_tipoalmacen=1) and sa.cod_chofer=$cod_personal";  
      }
      //echo $sql;
      $valor=0;
      require("conexion_comercial.php");
   ...
   function obtenerCodigoAreaciduad_comercial($codigo)
   { 
      $sql="SELECT cod_area from ciudades where cod_ciudad=$codigo";
      $value=0;
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
      while($row=mysqli_fetch_array($resp)){     
   ...
   function verificarRelacionComprobante($cod_ciudad,$fechaVenta){
      $dbh = new Conexion();
      $stmt = $dbh->prepare("SELECT c.codigo from ingresos_sucursales_comprobantes i join comprobantes c on i.cod_comprobante=c.codigo where c.cod_estadocomprobante<>2 and i.cod_ciudad=$cod_ciudad and i.fecha='$fechaVenta'");
      $value=0;
      $stmt->execute();
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $value=$row['codigo']; 
   ...
   function existeComprobanteRelacionadoingresosSuc($codigo){
      $sql="SELECT codigo from cierre_caja_diario where cod_comprobante=$codigo";
      $value=0;
      require("conexion_comercial.php");
      $resp=mysqli_query($dbh,$sql);
      while($row=mysqli_fetch_array($resp)){ 
   ...
   function obtenerCorrelativoingresoAlmacen(){
     $dbh = new Conexion();
     $stmt = $dbh->prepare("SELECT IFNULL(max(c.nro_correlativo)+1,1)as codigo from ingresos_almacen c");
     $stmt->execute();
     $codigo=1;
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $codigo=$row['codigo'];
   ...
   function obtenerGlosaIngresoAlmacen($codigo){
    $dbh = new Conexion();
    $stmt = $dbh->prepare("SELECT glosa From ingresos_almacen where codigo=$codigo");
     $stmt->execute();
     $valor="";
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['glosa'];
   ...
   function obtenerTotalIngresosAlmacen($codigo){
    $dbh = new Conexion();
    $stmt = $dbh->prepare("SELECT sum(monto_factura) as monto from ingresos_almacen_detalle where cod_ingresoalmacen=$codigo");
     $stmt->execute();
     $valor=0;
     while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['monto'];
   ...
   function verificarContabilizacion_ingresos_dcto($codigo){
      $dbh = new Conexion();
      $stmt = $dbh->prepare("SELECT codigo from comprobantes where cod_estadocomprobante<>2 and codigo in (select i.cod_comprobante from ingresos_almacen i join ingresos_almacen_detalle d on i.codigo=d.cod_ingresoalmacen
      where d.dcto_almacen=$codigo and i.cod_comprobante>0) ");
      $stmt->execute();
      $valor=0;
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['codigo'];
   ...
   function verificar_relacion_comprobante_pagoproveedores($codigo){
      $dbh = new Conexion();
      $sql="SELECT p.cod_comprobante 
      from pagos_lotes p join comprobantes c on p.cod_comprobante=c.codigo 
      where c.cod_estadocomprobante<>2 and p.codigo=$codigo";
      $stmt = $dbh->prepare($sql);
      // echo $sql;
      $stmt->execute();
      $valor=0;
   ...
   function verificar_relacion_comprobante_ingresoAlm($codigo){
      $dbh = new Conexion();
      $stmt = $dbh->prepare("SELECT cod_comprobante from ingresos_almacen where  codigo=$codigo");
      $stmt->execute();
      $valor=0;
      while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $valor=$row['cod_comprobante'];
...
function verificarComporbanteCojo($codigo){
   $dbh = new Conexion();
   $stmt = $dbh->prepare("SELECT  round(sum(cd.debe),2)-ROUND(sum(cd.haber),2) as diferencia from comprobantes_detalle cd where cd.cod_comprobante=$codigo and cd.cod_cuenta<>0 and cd.cod_unidadorganizacional<>0");
   $stmt->execute();
   $valor=1;
   while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
function reprocesar_costoventas_sucursales_2($fecha,$rpt_territorio){
   $sql="SELECT sum((select ct.costo_unitario from costoscobofar_2.costo_transaccion ct where ct.cod_material=sad.cod_material and ct.cod_documento=sa.cod_salida_almacenes and ct.cod_tipodocumento=0)*sad.cantidad_unitaria) as costo_venta 
      from salida_almacenes sa INNER JOIN salida_detalle_almacenes sad on sad.cod_salida_almacen=sa.cod_salida_almacenes
      where sa.fecha = '$fecha' and sa.cod_tiposalida=1001 and sa.salida_anulada=0 and sa.`cod_almacen` in (select a.`cod_almacen` from `almacenes` a
    where a.`cod_ciudad`='$rpt_territorio' and a.cod_tipoalmacen=1)";  
    // echo $sql."<br>";
...
function cargarValoresVentasYSaldosProductosArray_prodrotacion_prov($almacen,$fecha_ini,$fecha_fin,$proveedores){
    ...
    //ingresos
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0)),0),0)INGRESO,IFNULL(ROUND(sum(((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ant') and c.cod_gestion=YEAR('$fecha_ant') and c.cod_almacen=$almacen limit 1)),2),0)INGRESO_COSTO 
    ...
    //salidas
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)SALIDA,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ant') and c.cod_gestion=YEAR('$fecha_ant') and c.cod_almacen=$almacen limit 1)),2),0)SALIDA_COSTO  from salida_almacenes s, salida_detalle_almacenes sd,material_apoyo m,proveedores_lineas pl
    ...
    //para saldo actual
    //ingresos
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0)),0),0)INGRESO,IFNULL(ROUND(sum(((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=i.cod_ingreso_almacen and c.cod_tipodocumento=1 limit 1)),2),0)INGRESO_COSTO 
     from ingreso_almacenes i, ingreso_detalle_almacenes id,material_apoyo m,proveedores_lineas pl
    ...
    //salidas
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)SALIDA,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=s.cod_salida_almacenes and c.cod_tipodocumento=0 limit 1)),2),0)SALIDA_COSTO from salida_almacenes s, salida_detalle_almacenes sd,material_apoyo m,proveedores_lineas pl
    ...
    //VENTAS
    $tipoPago="1,2,3,4";
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)VENTAS,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=s.cod_salida_almacenes and c.cod_tipodocumento=0 limit 1)),2),0)VENTAS_COSTO FROM salida_detalle_almacenes sd join salida_almacenes s on s.cod_salida_almacenes=sd.cod_salida_almacen join material_apoyo m on m.codigo_material=sd.cod_material join proveedores_lineas pl on pl.cod_linea_proveedor=m.cod_linea_proveedor
...
function obtenerProveedor_presentacionAlmacen_nuevo(){       
 ...
 function cargarValoresVentasYSaldosProductosArray_prodrotacionProducto($almacen,$fecha_ini,$fecha_fin,$productos){
    ...
    //para saldo actual
    //ingresos
    $sql="select id.cod_material,IFNULL(ROUND(sum((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0)),0),0)INGRESO,IFNULL(ROUND(sum(((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=i.cod_ingreso_almacen and c.cod_tipodocumento=1 limit 1)),2),0)INGRESO_COSTO from ingreso_almacenes i, ingreso_detalle_almacenes id,material_apoyo m
      where i.cod_ingreso_almacen=id.cod_ingreso_almacen and m.codigo_material=id.cod_material and i.fecha>='$fecha_ini' and i.fecha<='$fecha_fin' and i.cod_almacen='$almacen'
      and id.cod_material in ($productos) and i.ingreso_anulado=0
      GROUP BY id.cod_material";
      // echo  $sql."<br>";
    $resp=mysqli_query($enlaceCon,$sql);
    while($row=mysqli_fetch_array($resp)){  
       $ingresos[$row['cod_material']]=$row['INGRESO']; 
       $ingresos_unidad[$row['cod_material']]=$row['INGRESO_COSTO'];       
    }
    //salidas
    $sql="select sd.cod_material,IFNULL(ROUND(sum(sd.cantidad_envase),0),0)SALIDA,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=s.cod_salida_almacenes and c.cod_tipodocumento=0 limit 1)),2),0)SALIDA_COSTO
    from salida_almacenes s, salida_detalle_almacenes sd, material_apoyo m
      where s.cod_salida_almacenes=sd.cod_salida_almacen and sd.cod_material=m.codigo_material and s.fecha>='$fecha_ini' and s.fecha<='$fecha_fin' and s.cod_almacen='$almacen'
      and sd.cod_material in ($productos) and s.salida_anulada=0 AND s.`cod_tiposalida`<>1001
      GROUP BY sd.cod_material";
       //echo  $sql."<br><br><br>*$productos**";
    $resp=mysqli_query($enlaceCon,$sql);
    while($row=mysqli_fetch_array($resp)){  
       $salida[$row['cod_material']]=$row['SALIDA'];   
       $salida_unidad[$row['cod_material']]=$row['SALIDA_COSTO'];        
    }

    //VENTAS
    $tipoPago="1,2,3,4";
    $sql="select sd.cod_material,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)VENTAS,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo_unitario from costoscobofar_2.costo_transaccion c where c.cod_material=m.codigo_material and c.cod_documento=s.cod_salida_almacenes and c.cod_tipodocumento=0 limit 1)),2),0)VENTAS_COSTO  FROM salida_detalle_almacenes sd join salida_almacenes s on s.cod_salida_almacenes=sd.cod_salida_almacen join material_apoyo m on m.codigo_material=sd.cod_material where sd.cod_salida_almacen=s.cod_salida_almacenes and sd.cod_material in ($productos) and s.`cod_tiposalida`=1001 and s.`cod_almacen` in ($almacen) and s.salida_anulada=0 and s.cod_tipopago in ($tipoPago) and s.`fecha` BETWEEN '$fecha_ini' and '$fecha_fin' GROUP BY sd.cod_material";
    //echo $sql;
    $resp=mysqli_query($enlaceCon,$sql);
    while($row=mysqli_fetch_array($resp)){    
       $ventas[$row['cod_material']]=$row['VENTAS'];
       $ventas_unidad[$row['cod_material']]=$row['VENTAS_COSTO'];    
    } 
...
function cargarValoresVentasYSaldosProductosArray_prodrotacion_provPromedio($almacen,$fecha_ini,$fecha_fin,$proveedores){
    ...
    //para saldo actual
    //ingresos
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0)),0),0)INGRESO,IFNULL(ROUND(sum(((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)INGRESO_COSTO 
     from ingreso_almacenes i, ingreso_detalle_almacenes id,material_apoyo m,proveedores_lineas pl
      where i.cod_ingreso_almacen=id.cod_ingreso_almacen and m.codigo_material=id.cod_material and m.cod_linea_proveedor=pl.cod_linea_proveedor and i.fecha>='$fecha_ini' and i.fecha<='$fecha_fin' and i.cod_almacen='$almacen'
      and pl.cod_proveedor in ($proveedores) and i.ingreso_anulado=0
      GROUP BY pl.cod_proveedor";
    ...
    //salidas
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)SALIDA,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)SALIDA_COSTO from salida_almacenes s, salida_detalle_almacenes sd,material_apoyo m,proveedores_lineas pl
      where s.cod_salida_almacenes=sd.cod_salida_almacen and m.codigo_material=sd.cod_material and m.cod_linea_proveedor=pl.cod_linea_proveedor and s.fecha>='$fecha_ini' and s.fecha<='$fecha_fin' and s.cod_almacen='$almacen'
      and pl.cod_proveedor in ($proveedores) and s.salida_anulada=0 AND s.`cod_tiposalida`<>1001 and (s.almacen_destino<>1078 or s.almacen_destino is null)
      GROUP BY pl.cod_proveedor";
    $resp=mysqli_query($enlaceCon,$sql);
    ...
    //VENTAS
    $tipoPago="1,2,3,4";
    $sql="select pl.cod_proveedor,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)VENTAS,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)VENTAS_COSTO FROM salida_detalle_almacenes sd join salida_almacenes s on s.cod_salida_almacenes=sd.cod_salida_almacen join material_apoyo m on m.codigo_material=sd.cod_material join proveedores_lineas pl on pl.cod_linea_proveedor=m.cod_linea_proveedor
     where sd.cod_salida_almacen=s.cod_salida_almacenes and pl.cod_proveedor in ($proveedores) and s.`cod_tiposalida`=1001 and s.`cod_almacen` in ($almacen) and s.salida_anulada=0 and s.cod_tipopago in ($tipoPago) and s.`fecha` BETWEEN '$fecha_ini' and '$fecha_fin' GROUP BY pl.cod_proveedor";
    $resp=mysqli_query($enlaceCon,$sql);
    while($row=mysqli_fetch_array($resp)){    
       $ventas[$row['cod_proveedor']]=$row['VENTAS'];
...
function cargarValoresVentasYSaldosProductosArray_prodrotacionProductoPromedio($almacen,$fecha_ini,$fecha_fin,$productos){
    ...
    //para saldo actual
    //ingresos
    $sql="select id.cod_material,IFNULL(ROUND(sum((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0)),0),0)INGRESO,IFNULL(ROUND(sum(((IFNULL(id.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(id.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)INGRESO_COSTO from ingreso_almacenes i, ingreso_detalle_almacenes id,material_apoyo m
      where i.cod_ingreso_almacen=id.cod_ingreso_almacen and m.codigo_material=id.cod_material and i.fecha>='$fecha_ini' and i.fecha<='$fecha_fin' and i.cod_almacen='$almacen'
      and id.cod_material in ($productos) and i.ingreso_anulado=0
      GROUP BY id.cod_material";
      // echo  $sql."<br>";
    ...
    //salidas
    $sql="select sd.cod_material,IFNULL(ROUND(sum(sd.cantidad_envase),0),0)SALIDA,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)SALIDA_COSTO
    from salida_almacenes s, salida_detalle_almacenes sd, material_apoyo m
      where s.cod_salida_almacenes=sd.cod_salida_almacen and sd.cod_material=m.codigo_material and s.fecha>='$fecha_ini' and s.fecha<='$fecha_fin' and s.cod_almacen='$almacen'
      and sd.cod_material in ($productos) and s.salida_anulada=0 AND s.`cod_tiposalida`<>1001
      GROUP BY sd.cod_material";
    ...
    //VENTAS
    $tipoPago="1,2,3,4";
    $sql="select sd.cod_material,IFNULL(ROUND(sum((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0)),0),0)VENTAS,IFNULL(ROUND(sum(((IFNULL(sd.cantidad_envase,0)*m.cantidad_presentacion)+IFNULL(sd.cantidad_unitaria,0))*(select c.costo from costoscobofar_2.costo_promedio_mes c where c.cod_material=m.codigo_material and c.cod_mes=MONTH('$fecha_ini') and c.cod_gestion=YEAR('$fecha_ini') and c.cod_almacen=$almacen limit 1)),2),0)VENTAS_COSTO  FROM salida_detalle_almacenes sd join salida_almacenes s on s.cod_salida_almacenes=sd.cod_salida_almacen join material_apoyo m on m.codigo_material=sd.cod_material where sd.cod_salida_almacen=s.cod_salida_almacenes and sd.cod_material in ($productos) and s.`cod_tiposalida`=1001 and s.`cod_almacen` in ($almacen) and s.salida_anulada=0 and s.cod_tipopago in ($tipoPago) and s.`fecha` BETWEEN '$fecha_ini' and '$fecha_fin' GROUP BY sd.cod_material";
    //echo $sql;
    $resp=mysqli_query($enlaceCon,$sql);
    while($row=mysqli_fetch_array($resp)){    
       $ventas[$row['cod_material']]=$row['VENTAS'];
