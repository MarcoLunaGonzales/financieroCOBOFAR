            ...
            <div class="table-responsive">
              <table class="table table-bordered table-condensed" width="100%" align="center"  id="tablePaginatorFixedPlanillaSueldo_otros">
                ...
                <tbody>
                  ...
                  $Tipo_contrato=1;
                  $Horas_pagadas=8;
                  $sql="SELECT prd.correlativo_planilla,(select tip.abreviatura from tipos_identificacion_personal tip where tip.codigo=p.cod_tipo_identificacion) as tipo_identificacion,p.identificacion,(select pd.abreviatura from personal_departamentos pd where pd.codigo=p.cod_lugar_emision)as lugar_emision,p.fecha_nacimiento,p.paterno,p.materno,p.primer_nombre,(select n.nombre from personal_pais n where n.codigo=p.cod_nacionalidad)as nacionalidad,(select g.abreviatura from tipos_genero g where g.codigo=p.cod_genero) as genero,p.jubilado,(select pd.tipo_persona_discapacitado from personal_discapacitado pd where pd.codigo=p.codigo)as tipo_personal_discapacidad,prd.ing_planilla,prd.retiro_planilla,p.cod_cajasalud,p.cod_tipoafp,p.nua_cua_asignado,(select c.nombre from cargos c where c.codigo=p.cod_cargo) as cargo,p.cod_unidadorganizacional,
