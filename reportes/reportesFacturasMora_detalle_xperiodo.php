...
$NombreGestion = $_GET["NombreGestion"];
$cuentax = $_GET["cuentai"];
$cuenta=explode(',', $cuentax);

// $sqlFechaEstadoCuenta=$_GET["sqlFechaEstadoCuenta"];
$StringUnidades=$_GET["StringUnidades"];
$cod_cuentaauxX=$_GET["cod_cuentaauxX"];
...
$proveedoresStringAux="and e.cod_cuentaaux in ($cod_cuentaauxX)";

// if(count($proveedores)==(int)$_GET["numero_proveedores"]){
...
$unidadCostoArray=$_GET['unidadCostoArray'];
$areaCostoArray=$_GET['areaCostoArray'];
                    ...
                    <div class="card-header card-header-icon">
                      ...
                      <h4 class="card-title"> 
                        <img  class="card-img-top"  src="../marca.png" style="width:100%; max-width:250px;">
                          Vencimiento de Facturas Detallado por Proveedor
                      </h4>
                            ...
                            $html='<table class="table table-bordered table-condensed" id="tablePaginatorFixedEstadoCuentas">'.
                                '<thead>'.
                                    '<tr class="">'.
                                        ...
                                        '<th class="text-left">Fecha Fact</th>'.
                                        '<th class="text-left">Fecha Venc</th>'.
                                        '<th class="text-left">Cliente</th>'.
                                        '<th class="text-left">Dias Mora</th>'.
                                ...
                                '<tbody>';
                                    ...
                                    foreach ($cuenta as $cuentai ) {
                                        ...
                                        while ($row = $stmtUO->fetch()) {
                                            ...
                                            $existeCuentas2=0;
                                            $stmtCantidad = $dbh->prepare("SELECT count(*) as cantidad FROM estados_cuenta e,comprobantes_detalle d, comprobantes cc, cuentas_auxiliares ca  where e.cod_comprobantedetalle=d.codigo and cc.codigo=d.cod_comprobante and e.cod_cuentaaux=ca.codigo and cc.cod_estadocomprobante<>2 and d.cod_cuenta in ($cuentai) and e.cod_comprobantedetalleorigen=0 and cc.cod_gestion= '$NombreGestion' and cc.fecha BETWEEN '$desde 00:00:00' and '$hasta 23:59:59' and cc.cod_unidadorganizacional in ($StringUnidades) and e.cod_cuentaaux in ($cod_cuentaauxX) and d.cod_unidadorganizacional in ($unidadCostoArray) and d.cod_area in ($areaCostoArray) and e.codigo=$codigoX order by ca.nombre, cc.fecha");
                                            $stmtCantidad->execute();
                                            while ($rowCantidad = $stmtCantidad->fetch()) {
                                            ...
                                            $montoX=$row['monto'];
                                            $fechaX=$row['fecha_factura'];
                                            $fecha_vencimientoX=$row['fecha_vencimiento'];
                                            $fechaX=strftime('%d/%m/%Y',strtotime($fechaX));
                                            $fecha_vencimientoX=strftime('%d/%m/%Y',strtotime($fecha_vencimientoX));
                                            ...
                                            // $nroFacturaOficial=$row['nro_factura'];
                                            $nroFacturaOficial="";
                                            ...
                                            if($glosaMostrar>72){
                                               $glosaMostrar=substr($glosaMostrar, 0, 72)."...";
                                            }
                                            ...
                                            if(isset($_GET['cierre_posterior'])){
                                            ...
                                            //     $diasTranscurridos=1000;
                                            // }
                                            $sw_bandera=false;
                                            if($dias1==-100 && $dias2=-100 && $diasTranscurridos==null){
                                                $sw_bandera=true;
                                            }

                                        if(($diasTranscurridos>=$dias1 && $diasTranscurridos<$dias2) || $sw_bandera){
                                    ...
                                    $html.='<tr>                                            
                                        ...
                                        <td style="display: none;"></td>
                                        <td style="display: none;"></td>
                                        <td class="text-right small" colspan="8">Total:</td>
