...
$sql="SELECT i.cod_ingreso_almacen,(select a.nombre_almacen from almacenes a where a.cod_almacen= i.cod_almacen) as almacen,(select ts.nombre_tipoingreso from tipos_ingreso ts where ts.cod_tipoingreso=i.cod_tipoingreso)as tipo_ingreso,i.cod_tipo_doc,i.fecha,i.nro_correlativo,(select m.descripcion_material from material_apoyo m where m.codigo_material=id.cod_material)as material,id.cod_material,id.cantidad_unitaria,id.cantidad_envase,id.precio_bruto,id.costo_almacen,(SELECT costo_unitario from costoscobofar.costo_transaccion where cod_almacen=i.cod_almacen and cod_tipodocumento=1 and cod_documento=i.cod_ingreso_almacen and cod_material=id.cod_material) as costo_unitario 
from ingreso_detalle_almacenes id join ingreso_almacenes i on id.cod_ingreso_almacen=i.cod_ingreso_almacen
where id.cod_material>0 and i.ingreso_anulado=0
and i.fecha BETWEEN '$desde' and '$hasta' having costo_unitario=0
order by i.fecha";
 // echo $sql;
?>
<div class="content">
          ...
          <div class="card-body">
            ...
            $html='<table class="table table-bordered table-condensed" width="100%" align="center" id="libro_mayor_rep">'.
              ...
              while($row=mysqli_fetch_array($resp)){
                ...
                '<td class="text-left small">'.$row['nro_correlativo'].'('.$row['cod_ingreso_almacen'].')</td>'.
