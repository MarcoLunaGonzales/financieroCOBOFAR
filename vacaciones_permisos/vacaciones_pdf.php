...
$html.=  '<header class="header">'.
            ...
            '<table border="1" align="center" style="width: 100%;border-collapse: collapse;">'.
                '<tbody style=" font-family: Times New Roman;
                    ...
                    <tr style="background:#45b39d;color:black;"><td></td><td align="center">F.INICIO</td><td align="center">F.FIN</td><td align="center">TOTAL DIAS</td><td align="center">COMENTARIOS</td><td align="center">SALDO</td></tr>';
                    $saldo_total=0;
                    for ($i=0; $i <count($datos) ; $i++) { 
                        ...
                        $sql="SELECT fecha_inicial,hora_inicial,fecha_final,hora_final,observaciones,dias_vacacion
                            from personal_vacaciones where cod_personal=$cod_personal and cod_estadoreferencial=1 and gestion=$nombre_gestion";                        
                        $stmt = $dbh->prepare($sql);
                        $stmt->execute();
                        
                        $contador_det=0;
                        $html.='<tr><td align="center" rowspan="'.$total_det.'">GESTION<BR><span style="font-size:18px">'.$nombre_gestion.'</span><BR>ACUMULADO <b>'.$acumulado_gestion.'</b> DIAS</td>';
                        while ($result = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $contador_det++;
                            $html.='<td align="center">'.$fecha_inicial.'</td><td align="center">'.$fecha_final.'</td><td align="center">'.round($dias_vacacion).'</td><td>'.$observaciones.'</td><td align="center"><h2>'.$saldo_gestion.'</h2></td></tr>';
                        }
                        if($contador_det==0){
            ...
            $html.='<br><br><br><br><br><br><table width="100%">
                          ...
                          '<td><center><p>______________________________<BR>'.$paterno.' '.$materno.' '.$primer_nombre.'</p></center></td>
                          </tr>
                        </table>';
            $html.='</header>'.
        '</body>'.
